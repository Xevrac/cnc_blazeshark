...
namespace BlazeSharkWV
{
    public partial class Form1 : Form
    {
        ...
        private BackgroundWorker proxyWorker;
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            proxyWorker = new BackgroundWorker();
            proxyWorker.WorkerSupportsCancellation = true;
            proxyWorker.DoWork += ProxyWorker_DoWork;
            proxyWorker.RunWorkerCompleted += ProxyWorker_RunWorkerCompleted;
        ...
        private void StopProxyMain()
        {
            if (proxyWorker.IsBusy)
            {
                proxyWorker.CancelAsync();
                SetExit(true);  // Signal other threads to stop
            }
        }

        private void ProxyWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                while (!GetExit())
                {
                    if (proxyWorker.CancellationPending)
                    {
                        e.Cancel = true;
                        return;
                    }
                    ...
                    {
                        // If no pending connection, sleep for a short time to avoid high CPU usage
                        Thread.Sleep(100);
                    }
                }
            }
        ...
        private void ProxyWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            // This method will be called when the background worker completes its task
            toolStripButton1.Enabled = true;
            Log("Stopping...");
        }
